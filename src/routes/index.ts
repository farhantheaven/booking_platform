...
export const createRoutes = (db: DatabaseConnection): Router => {
  const router = Router();

  // Initialize services
  const recurrenceService = new RecurrenceService();
  const conflictService = new ConflictDetectionService(db, recurrenceService);
  const availabilityService = new AvailabilityService(db, conflictService, recurrenceService, conflictService);
  const bookingService = new BookingService(db, recurrenceService, conflictService, availabilityService);
  ...
  // Booking exceptions routes
  router.post('/bookings/:id/exceptions', bookingController.addBookingException);
  router.get('/bookings/:id/exceptions', bookingController.getBookingExceptions);
  router.put('/bookings/:id/exceptions/:exceptionId', bookingController.updateBookingException);
  router.delete('/bookings/:id/exceptions/:exceptionId', bookingController.deleteBookingException);
